package com.example.backend_tutorial.integration.momo.config;
...
public class MoMoEnvironment {
    ...
    public MoMoEnvironment(MoMoEndpoint endpoints, PartnerInfo partnerInfo, EnvTarget target) {
        this(endpoints, partnerInfo, target.string());
    }

    public MoMoEnvironment(MoMoEndpoint momoEndpoint, PartnerInfo partnerInfo, String target) {
    ...
    public static MoMoEnvironment selectEnv(EnvTarget target) {
        try (InputStream input = MoMoEnvironment.class.getClassLoader().getResourceAsStream("application.properties")) {
            ...
            switch (target) {
                case DEV:
                    ...
                    MoMoEnvironment dev = new MoMoEnvironment(devEndpoint, devInfo, target);
                    return dev;
                case PROD:
                    ...
                    MoMoEnvironment prod = new MoMoEnvironment(prodEndpoint, prodInfo, target);
                    return prod;
                default:
                    throw new IllegalArgumentException("MoMo doesnt provide other environment: dev and prod");
            }
